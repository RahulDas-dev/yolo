...
def load_weights(model, weights_file_path):
    ...
    with open(weights_file_path, "rb") as file:
        ...
        for conv_idx in range(conv_layer_size):
            ...
            bn_layer_name = (
                f"batch_normalization_{bn_idx}" if bn_idx > 0 else "batch_normalization"
            )
...
def draw_bbox(
    ...
    for _, row in detections.iterrows():
        ...
        if show_text:
            ...
            cv2.rectangle(
                img,
                (x1 - line_width // 2, y1 - text_height),
                (x1 + text_width, y1),
                color,
                cv2.FILLED,
            )
...
class DataGenerator(Sequence):
    ...
    def __init__(
        ...
        self.num_classes = len(
            [line.strip() for line in open(class_name_path).readlines()]
        )
    ...
    def __data_generation(self, annotation_lines):
        ...
        y_bbox = np.empty(
            (len(annotation_lines), self.max_boxes, 5), dtype=np.float32
        )  # x1y1x2y2
        ...
        y_tensor, y_true_boxes_xywh = preprocess_true_boxes(
            y_bbox, self.target_img_size[:2], self.anchors, self.num_classes
        )
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    true_boxes_xy = (
        true_boxes_abs[..., 0:2] + true_boxes_abs[..., 2:4]
    ) // 2  # (100, 2)
    ...
    grid_sizes = [
        input_shape // {0: 8, 1: 16, 2: 32}[stage] for stage in range(num_stages)
    ]
    y_true = [
        np.zeros(
            (bs, grid_sizes[s][0], grid_sizes[s][1], bbox_per_grid, 5 + num_classes),
            dtype="float32",
        )
        ...
        for box_idx in range(num_boxes):
            ...
            for stage in range(num_stages):
                if best_anchor in anchor_mask[stage]:
                    ...
                    y_true[stage][
                        batch_idx, grid_row, grid_col, anchor_idx, :2
                    ] = true_boxes_xy[
                        batch_idx, box_idx, :
                    ]  # abs xy
                    y_true[stage][
                        batch_idx, grid_row, grid_col, anchor_idx, 2:4
                    ] = true_boxes_wh[
                        batch_idx, box_idx, :
                    ]  # abs wh
                    y_true[stage][
                        batch_idx, grid_row, grid_col, anchor_idx, 4
                    ] = 1  # confidence

                    y_true[stage][
                        batch_idx, grid_row, grid_col, anchor_idx, 5 + class_idx
                    ] = 1  # one-hot encoding
                    # smooth
