...
def yolo_loss(args, num_classes, iou_loss_thresh, anchors):
    ...
    sbbox_ciou_loss, sbbox_conf_loss, sbbox_prob_loss = loss_layer(
        conv_sbbox,
        pred_sbbox,
        label_sbbox,
        true_bboxes,
    ...
    mbbox_ciou_loss, mbbox_conf_loss, mbbox_prob_loss = loss_layer(
        conv_mbbox,
        pred_mbbox,
        label_mbbox,
        true_bboxes,
    ...
    lbbox_ciou_loss, lbbox_conf_loss, lbbox_prob_loss = loss_layer(
        conv_lbbox,
        pred_lbbox,
        label_lbbox,
        true_bboxes,
...
def loss_layer(conv, pred, label, bboxes, stride, num_class, iou_loss_thresh):
    ...
    # Coordinate loss
    ciou = tf.expand_dims(
        bbox_giou(pred_xywh, label_xywh), axis=-1
    )  # (8, 13, 13, 3, 1)
    ...
    expand_pred_xywh = pred_xywh[
        :, :, :, :, np.newaxis, :
    ]  # (?, grid_h, grid_w, 3, 1, 4)
    expand_bboxes = bboxes[
        :, np.newaxis, np.newaxis, np.newaxis, :, :
    ]  # (?, 1, 1, 1, 70, 4)
    iou = bbox_iou(
        expand_pred_xywh, expand_bboxes
    )  # IoU between all pred bbox and all gt (?, grid_h, grid_w, 3, 70)
    max_iou = tf.expand_dims(
        tf.reduce_max(iou, axis=-1), axis=-1
    )  # max iou: (?, grid_h, grid_w, 3, 1)
