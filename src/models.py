...
from custom_layers import nms, yolov4_head, yolov4_neck
...
class Yolov4(object):
    def __init__(
        ...
        assert (
            config["img_size"][0] % config["strides"][-1] == 0
        ), "must be a multiple of last stride"
        ...
        self.class_color = {
            name: list(np.random.random(size=3) * 255) for name in self.class_names
        }
        # Training
    ...
    def build_model(self, load_pretrained=True):
        ...
        y_true = [
            layers.Input(
                name="input_2", shape=(52, 52, 3, (self.num_classes + 5))
            ),  # label small boxes
            layers.Input(
                name="input_3", shape=(26, 26, 3, (self.num_classes + 5))
            ),  # label medium boxes
            layers.Input(
                name="input_4", shape=(13, 13, 3, (self.num_classes + 5))
            ),  # label large boxes
        ...
        yolov4_output = yolov4_head(
            yolov4_output, self.num_classes, self.anchors, self.xyscale
        )
        ...
        if (
            ...
            and self.weight_path.endswith(".weights")
        ):
    ...
    def load_model(self, path):
        ...
        yolov4_output = yolov4_head(
            self.yolo_model.output, self.num_classes, self.anchors, self.xyscale
        )
        self.inference_model = models.Model(
    ...
    def fit(
        self, train_data_gen, epochs, val_data_gen=None, initial_epoch=0, callbacks=None
    ):
    ...
    def predict_img(
        ...
        detections = get_detection_data(
            img=raw_img, model_outputs=pred_output, class_names=self.class_names
        )
    ...
    def export_gt(self, annotation_path, gt_folder_path):
        with open(annotation_path) as file:
            for line in file:
                ...
                # export txt file
                with open(
                    os.path.join(gt_folder_path, filename + ".txt"), "w"
                ) as output_file:
                    for obj in objs:
                        x_min, y_min, x_max, y_max, class_id = [
                            float(o) for o in obj.strip().split(",")
                        ]
                        output_file.write(
                            f"{self.class_names[int(class_id)]} {x_min} {y_min} {x_max} {y_max}\n"
                        )

    def export_prediction(
        self, annotation_path, pred_folder_path, img_folder_path, bs=2
    ):
        with open(annotation_path) as file:
            img_paths = [
                os.path.join(img_folder_path, line.split(" ")[0].split(os.sep)[-1])
                for line in file
            ]
            # print(img_paths[:20])
            for batch_idx in tqdm(range(0, len(img_paths), bs)):
                ...
                (
                    ...
                    b_classes,
                    b_valid_detections,
                ) = self.inference_model.predict(imgs)
                for k in range(len(paths)):
                    ...
                    with open(output_path, "w") as pred_file:
                        for box_idx in range(num_boxes):
                            b = boxes[box_idx]
                            pred_file.write(
                                f"{cls_names[box_idx]} {scores[box_idx]} {b[0]} {b[1]} {b[2]} {b[3]}\n"
                            )

    def eval_map(
        self, gt_folder_path, pred_folder_path, temp_json_folder_path, output_files_path
    ):
        """Process Gt"""
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            with open(
                temp_json_folder_path + "/" + class_name + "_dr.json", "w"
            ) as outfile:
                json.dump(bounding_boxes, outfile)
        ...
        if True:
            ...
            plot_title += (
                str(count_non_zero_values_in_dictionary) + " detected classes)"
            )
            # end Plot title
            x_label = "Number of objects per class"
    ...
    def predict_nonms(self, img_path, iou_threshold=0.413, score_threshold=0.1):
        ...
        pred_output = nms(
            output, self.img_size, self.num_classes, iou_threshold, score_threshold
        )
