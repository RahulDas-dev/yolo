...
def conv(
    x, filters, kernel_size, downsampling=False, activation="leaky", batch_norm=True
):
    def mish(x):
...
def yolov4_head(yolo_neck_outputs, classes, anchors, xyscale):
    bbox0, object_probability0, class_probabilities0, pred_box0 = get_boxes(
        yolo_neck_outputs[0],
        anchors=anchors[0, :, :],
        classes=classes,
        grid_size=52,
    ...
    bbox1, object_probability1, class_probabilities1, pred_box1 = get_boxes(
        yolo_neck_outputs[1],
        anchors=anchors[1, :, :],
        classes=classes,
        grid_size=26,
    ...
    bbox2, object_probability2, class_probabilities2, pred_box2 = get_boxes(
        yolo_neck_outputs[2],
        anchors=anchors[2, :, :],
        classes=classes,
        grid_size=13,
...
def get_boxes(pred, anchors, classes, grid_size, strides, xyscale):
    ...
    """
    pred = tf.reshape(
        pred, (tf.shape(pred)[0], grid_size, grid_size, 3, 5 + classes)
    ...
    box_xy = (
        (box_xy * xyscale) - 0.5 * (xyscale - 1) + grid
    ) * strides  # (?, 52, 52, 1, 4)
...
def nms(model_ouputs, input_shape, num_class, iou_threshold=0.413, score_threshold=0.3):
    ...
    for output_idx in range(0, len(model_ouputs), 4):
        ...
        confidence = tf.concat(
            [confidence, tf.reshape(output_conf, (bs, -1, 1))], axis=1
        )
        class_probabilities = tf.concat(
            [class_probabilities, tf.reshape(output_classes, (bs, -1, num_class))],
            axis=1,
        )
